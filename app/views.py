...
CONFIG_FILE = os.path.join('config.yml').replace("\\","/")
...
def config():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('config')),form.data
    for c in config:
...
def readCase():
    if request.method == "GET":
        ...
        result = {}
        try:
            with open(caseName, 'r',encoding='utf-8') as f:
...
def runCase():
    if request.method == "POST":
        ...
        try:
            ...
            # # =======================
            command = 'nosetests -v {0}  --html-report="{1}"'.format(caseFiles, report_file.encode('gbk'))
            # 如果你本地无法通过 nosetests 得到想要的，亦可以考虑转 unittest 模块
            # # =======================
